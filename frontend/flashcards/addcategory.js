...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const addFlashcardButton = document.getElementById('addflashcard-btn');
    ...
    if (fileInput && uploadButton) {
        ...
        console.warn("File input or upload button not found.");
    }
    ...
    window.previewImage = function (event) {
        ...
        reader.onload = function () {
            ...
            if (photo) {
                ...
                console.warn("Photo element not found for image preview.");
            }
        };
    ...
    if (categoryForm) {
        categoryForm.addEventListener('submit', function (event) {
            ...
            console.log("Submitting category:", { categoryName, categoryDesc });
            ...
            fetch('http://localhost:3000/api/add-category', {
                ...
                .then((data) => {
                    ...
                    if (data.category_id) {
                        const createdCategoryId = data.category_id;
                        console.log("Stored categoryId:", createdCategoryId);
                        localStorage.setItem('createdCategoryId', createdCategoryId);
                        window.location.href = 'flashcards.html'; // Redirect to the noteapp.html page


                        if (addFlashcardButton) {
                            addFlashcardButton.style.display = 'block';
                        } else {
                            console.warn("Add flashcard button not found.");
                        }
                    } else {
                        console.error("Response missing category_id.");
                    }
                })
                .catch((error) => {
