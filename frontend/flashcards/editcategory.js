...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!categoryId) {
        ...
        window.location.href = 'categories.html'; // Redirect if no category ID is found
        return;
    }
    ...
    fetch('/api/user/profile')
        ...
        .then(user => {
            ...
            // After the user details are fetched, fetch the category details
...
function fetchCategoryDetails(categoryId) {
    fetch(`/api/category/${categoryId}`)
        ...
        .then(category => {
            ...
            // Ensure that the form elements exist before setting the values
            const categoryNameInput = document.getElementById('category_name');
            const categoryDescInput = document.getElementById('category_desc');

            if (categoryNameInput) {
                categoryNameInput.value = category.name; // Set the name
            }
            if (categoryDescInput) {
                categoryDescInput.value = category.category_desc || ''; // Set the description or leave empty
            }
        })
        .catch(error => {
...
document.getElementById('edit-category-form').addEventListener('submit', (event) => {
    ...
    const categoryDesc = document.getElementById('category_desc').value || null; // Allow empty description
    ...
    fetch(`/api/category/edit/${categoryId}`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
            name: categoryName,
            category_desc: categoryDesc, // Send null if description is empty
        }),
    })
        .then(response => {
            if (!response.ok) {
        ...
        .then(updatedCategory => {
            ...
            window.location.href = 'flashcards.html'; // Redirect to categories page
        })
        .catch(error => {
